...
> GREEN: The permissive port now binds to loopback inside the proxy container, so an enclave peer cannot reach it by the proxy's network address.[^31] Its access rules remain permissive for a process inside that container,[^32][^33] and the decision to use it for builds or remove it remains open.[^35][^36]
...
The GitHub token never enters the agent container: the mirror holds short-lived role credentials, relays pushes on the agent's behalf, and passes the credential on stdin rather than argv, closing a process-listing leak. The relay can invoke its helper for an HTTPS origin,[^48] but the helper now checks the credential request's protocol and exact host before returning a token.[^49]
...
[^8]: The false-pass scanner: advisory, no caller gates on it, skips over half the corpus, keyed on a non-zero exit | scripts/scan-litmus-false-pass.sh#L8-L48
...
[^13]: Launcher generates self-signed RSA-2048 CA with a 30-day life | crates/tillandsias-headless/src/main.rs#L3257-L3278
...
[^21]: The ledger's finding that such provenance verifies with no verifier identity, and its recommendation of bundles plus a pinned root — status ready, not adopted | plan/index.yaml#L12562
...
[^31]: The permissive port binds only to loopback in the proxy container | images/proxy/squid.conf#L70-L78
    > http_port 127.0.0.1:3129 ssl-bump
[^32]: Permissive port: allow everything, no source restriction | images/proxy/squid.conf#L146-L149
...
[^37]: Denied runtime traffic gets a TCP reset rather than a 403 | images/proxy/squid.conf#L148-L153
...
[^39]: The owner-only clamp on the CA key, in the stable release's source | crates/tillandsias-headless/src/main.rs#L3161-L3165
...
[^40]: On non-unix targets the clamp is a documented no-op | crates/tillandsias-headless/src/main.rs#L3181-L3185
...
[^41]: Pre-fix keys are healed down to owner-only on every pass | crates/tillandsias-headless/src/main.rs#L3362-L3366
...
[^42]: Historical report that two hosts installed the earlier key fix | plan/index.yaml#L63081-L63085
...
[^45]: Filed packet: make the CA directory private by construction and retire the clamp | plan/index.yaml#L28828
    > scripts/clamp-ca-material.sh gains a retirement condition or is deleted
[^46]: The authenticated push lane is gated on an environment variable that defaults to off | crates/tillandsias-headless/src/main.rs#L11490-L11494
...
[^50]: The vault container carries an embedded SELinux policy module | crates/tillandsias-headless/src/vault_bootstrap.rs#L2343-L2353
...
[^52]: On a rootless Linux host the vault too runs unconfined | crates/tillandsias-headless/src/vault_bootstrap.rs#L2434-L2452
    > `label=disable` runs the vault container unconfined on the host
[^53]: The vault launch adds a capability back and omits `--rm` | crates/tillandsias-headless/src/vault_bootstrap.rs#L2649-L2661
    > "--cap-drop".into(), "ALL".into(), "--cap-add".into(), "IPC_LOCK".into(),
[^54]: The approved bar-raise on litmus quality is diff-scoped by construction | plan/index.yaml#L14777
...
[^55]: Serialising a launch now refuses an argv that violates the envelope, in every build profile | crates/tillandsias-podman/src/container_spec.rs#L393-L397
    > crate::policy::validate_launch_argv(&argv)?; Ok(argv)
[^56]: The attached forge launch refuses on a hardening violation | crates/tillandsias-headless/src/main.rs#L13269
    > "refusing to launch {container_name}: hardening envelope violation: {err}"
[^57]: The delegated launch path refuses too | crates/tillandsias-headless/src/main.rs#L13097
...
[^61]: The build gate fails on it | build.sh#L4532
...
[^64]: A host-browser container still launches through the unchecked serialiser | crates/tillandsias-headless/src/main.rs#L12687
    > let args = spec.build_run_args();
[^65]: The repo's own admission that enforcement depends on call order | crates/tillandsias-headless/src/main.rs#L13076-L13080
...
[^74]: The Windows lane's coverage check, why it follows the transitive path, and the allow-list it blames for an installer shipping bare | .github/workflows/release.yml#L754-L785
...
[^77]: The macOS installer likewise | scripts/install-macos.sh#L115-L137
...
[^81]: Application reset refuses to reprovision if host credentials cannot be cleared | crates/tillandsias-headless/src/main.rs#L9834-L9850
    > --reset-state could not clear:
[^83]: Where no keyring is available the share is written to a fallback file in the cache directory | crates/tillandsias-headless/src/vault_bootstrap.rs#L3136-L3158
    > using fallback file (expected in VM guest and headless environments)
[^84]: Inside a VM guest both fallback writes are always attempted | crates/tillandsias-headless/src/vault_bootstrap.rs#L1343-L1370
...
[^85]: The "share present in keychain" check falls through to the file | crates/tillandsias-headless/src/vault_bootstrap.rs#L1412-L1440
...
[^86]: The preserve guard and the log line the smoke runs observed | crates/tillandsias-headless/src/vault_bootstrap.rs#L2528-L2548
...
[^95]: The observatorium web launch serialises without the policy call | crates/tillandsias-headless/src/main.rs#L12638
    > .build_run_args()
[^96]: The proxy launch is assembled as a raw argv the checker never sees | crates/tillandsias-headless/src/main.rs#L3517
    > fn build_proxy_run_args(certs_dir: &Path, image: &str) -> Vec<String> {
[^97]: The mirror launch is assembled the same way | crates/tillandsias-headless/src/main.rs#L4551
...
[^102]: CA directory creation uses ordinary create_dir_all, followed by refresh decision | crates/tillandsias-headless/src/main.rs#L3213-L3221
